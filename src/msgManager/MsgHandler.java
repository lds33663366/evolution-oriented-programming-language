 ...
 */
public class MsgHandler implements Serializable, TopicSubscriber {
	...
	public void obtainMessage() {
		...
		if ((actionName = detectMessage()) != null) {
			...
			CopyOnWriteArrayList<Message> mList = action_message
					.get(actionName);
			if (mList == null)
				mList = new CopyOnWriteArrayList<Message>();
			mList.add(m);
	...
	public void obtainTopicMessage(Message msg) {

		if (msg.getFrom().equals(instance.getIdName())) {
//			System.out.println("这条消息是自己发的，扔掉！");
			return;
		}
	...
	private Message searchMessage(String messageName) {
		...
		if (messageList == null)
			throw new RuntimeException("该instance没有要发送的消息");
	...
	private String detectMessage() {
		...
		// 判断是否是需要的消息
		else if ((actionName = relation.detectRelation(m.getName(),
	...
	public void subscribe(String topic, String actionName) {
	...
	public boolean unsubscribe(String topic) {
		return mp.removeListener(topic, this);
	}

	public boolean unsubscribeAll() {

		for (Iterator<Entry<String, String>> iter = topic_action.entrySet()
				.iterator(); iter.hasNext();) {
			Entry<String, String> entry = iter.next();
			String topic = entry.getKey();
			if (!unsubscribe(topic)) return false;
			System.out.println(instance.getIdName() + "取消订阅" + topic);
		}
